...
- `style.css` — all styling (colors, fonts, animations, responsive breakpoints)
- `script.js` — countdown timer, multi-step form, interactive tribe map, form submission
...
Open `script.js` and find the `CONFIG` object near the top. Pick ONE option:
...
- Replace each `src="https://placehold.co/..."` with your real photo path (e.g. drop images into an `/images` folder and reference `images/day1.jpg`)
- Replace `data-video="REPLACE_WITH_YOUTUBE_ID"` with your real YouTube video ID

## 4. Adjust dates & pricing

- Early bird countdown target: `CONFIG.earlyBirdDeadline` in `script.js`
